...
#### ГАЙД АКТУАЛЕН НА КОНЕЦ ЛЕТА 2026 ГОДА
...
Скачав ту же версию прошивки, что стоит сейчас, на свой пк, открывай её через любой архиватор (7zip, WinRAR, но ***НЕ ВИНДОВЫЙ***) и ищи внутри файл init-boot.img (boot.img, если ramdisk находится там и/или у вас есть только init-boot.img; или init'а вообще нет в прошивке, что бывает крайне редко)
...
После этого, переходим в [репозиторий KSUN](https://github.com/KernelSU-Next/KernelSU-Next/releases/latest) и устанавливаем последний стабильный релиз менеджера. Заходим и патчим наш образ init-boot.img в менеджере. На выходе получим файл в /sdcard/Download/, который смело отправляем на пк.
...
Универсальная команда для прошивки ядра на всех нормальных и околонормальных девайсах:
```
...
- Metamodule - модуль системы монтирования. После выхода KernelSU v3.0.0 является обязательным для некоторых случаях. В базовом скрытии не участвует
...
Настраивать всё вручную — затея долгая и бессмысленная. В тег [`configs`](https://github.com/mrvokintos/RootHide/releases/tag/configs) этого репозитория я закинул готовые конфиги для HMA и TEESimulator. Тебе остается их просто применить.
